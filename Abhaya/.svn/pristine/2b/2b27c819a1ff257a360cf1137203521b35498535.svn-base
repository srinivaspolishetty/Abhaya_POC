...
export class SuspiciousVehiclesComponent implements OnInit {
  ...
  removeSuspiciousVehicle(vehicle){
    ...
    .subscribe((data: any) => {
      console.log(data);
      this.getAllVehicles(0);
      this.toasterService.showSuccess('bottom-right', 'Successfully removed Vehicle Number : '+vehicle.rcNumber+' from Suspicious Vehicles List.'); 
    }, error => {
      console.log('Error', error);
      this.toasterService.showError('bottom-right', 'Unable to remove Vehicle Number: '+vehicle.rcNumber+' from Suspicious Vehicles List.'); 
    });
  }
