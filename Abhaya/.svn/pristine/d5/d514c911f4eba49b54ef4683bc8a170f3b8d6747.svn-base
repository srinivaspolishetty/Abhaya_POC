...
export class PanicAlertComponent implements OnInit {
  ...
  constructor(private router: Router, private driverApiService: DriverApiService, private activatedRoute: ActivatedRoute, private panicApiService: PanicApiService,
  ...
  ngOnInit() {
    this.activatedRoute.queryParams.subscribe(params => {
      ...
      // this.getPanicDetails(this.distressId);
  ...
  // getPanicDetails(tripID) {
  //   this.panicApiService.getPanicDetailsById(tripID).subscribe((data: PanicAlert) => {
  //     this.panicAlertArray.push(data);
  //     this.isClosed = this.panicAlertArray[0].closed
