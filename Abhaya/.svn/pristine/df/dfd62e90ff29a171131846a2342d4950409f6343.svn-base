...
export class PanicAlertsListComponent implements OnInit {
  ...
  openPanicAlerts(panicAlert) {
    this.router.navigate(['/rta/panicalert'], { queryParams: {distressId: panicAlert.distressId,tripId: panicAlert.tripId,
       rcNumber: panicAlert.rcNumber, citizenMobileNumber: panicAlert.citizenMobileNumber, orgin:panicAlert.sourceLatLang, destination:panicAlert.destiLatLang}, queryParamsHandling: 'merge' });
  }
  ...
  getPanicAlerts(page: number, searchValue: string, isClosed: boolean, districtId: string, cityId: string, eventType: string, searchDate:string) {
    this.isClosed = isClosed;
    this.panicApiService.getAll(page, 10, searchValue, isClosed, districtId, cityId, eventType, searchDate).subscribe((data:  PaginationModel) => {
