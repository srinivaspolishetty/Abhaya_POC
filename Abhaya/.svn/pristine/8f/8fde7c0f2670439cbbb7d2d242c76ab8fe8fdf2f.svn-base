...
export class DriversComponent implements OnInit {
  ...
  constructor(private driverApiService: DriverApiService, private modalService: NgbModal, private activatedRoute: ActivatedRoute,
    private mastersServicesApi: MastersServicesApi, private calendar: NgbCalendar, private ngbDateFRParserFormatter: NgbDateFRParserFormatter) {
  ...
  downloadDrivers(page: number, searchValue: string, districtId: string, cityId: string) {
    this.driverApiService.getAll(page, '', searchValue, districtId, cityId).subscribe((data:  PaginationModel) => {
      ...
      itemNew.forEach(element => {
        const temp = [element.driverId, element.dlNumber, element.driverName, element.rfId, element.contactNumber, element.gender, element.cityName, element.districtName, element.dlExpiryDate];
        rows.push(temp);
      });
