...
export class VehiclesComponent implements OnInit {
  ...
  openRegistration(vehicle) {
    const modalRef = this.modalService.open(NewregistrationComponent, { windowClass: 'popup_alignment_dashBoard', backdrop: 'static',keyboard: false });
    modalRef.componentInstance.vehicleNumber = vehicle.rcNumber;
  }
  openVehiclemap(vehicle) {
    const modalRef = this.modalService.open(VehiclemapComponent, { windowClass: 'popup_alignment_dashBoard', backdrop: 'static',
    keyboard: false });
  ...
  downloadVehicleDetails(page: number, searchValue: string, districtId: string, cityId: string) {
    this.vehicleApiService.getAll(page, '', searchValue, districtId, cityId, '').subscribe((data:  PaginationModel) => {
      ...
      itemNew.forEach(element => {
        const temp = [element.rcNumber, element.serialNumber, element.ownerName, element.ownerContactNumber, element.cityName, element.districtName, element.createdDate, element.registrationDate, element.rcExpiryDate];
        rows.push(temp);
      });
