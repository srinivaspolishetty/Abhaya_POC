...
export class RouteDeviationComponent implements OnInit {
  ...
  getRouteDeviations(page: number, searchValue: string, selectDate: string) {
    ...
    this.routeDeviationService.getAll(page, 10, searchValue, selectDate, this.districtModel, this.cityModel).subscribe((data: PaginationModel) => {
  ...
  downloadRouteDeviationTable() {
    ...
    this.routeDeviationService.getAll(0, '', '', this.formatDate, this.districtModel, this.cityModel).subscribe((data: PaginationModel) => {
      ...
      const col = ['Trip ID', 'Vehicle Number', 'RF ID', 'Trip Request ID', 'Serial No', 'Driver Name', 'DL Number', 'Driver Number', 'Owner Name', 'Owner Number', 'Citizen Mobile No', 'Deviation Time', 'Deviation Location', 'Source', 'Destination'];
      const rows = [];
      const itemNew = this.downloadrouteDeviationArray;
      console.log(this.downloadrouteDeviationArray);
      itemNew.forEach(element => {
        const temp = [element.tripId, element.rcNumber, element.rfId, element.tripRequestId, element.serialNumber, element.driverName, element.dlNumber, element.driverContactNumber, element.ownerName, element.ownerContactNumber, element.citizenMobileNumber, element.deviationTime, element.geoLocation, element.slocation, element.dlocation];
        rows.push(temp);
      });
